...
export function PublicFooter({ navItems, categories, siteName }: PublicFooterProps) {
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                {navItems.map((item) => (
                  <li key={item.label + item.value}>
                    <Link href={navItemHref(item)} className="hover:text-foreground transition-colors">
                      {item.label}
                    </Link>
                  </li>
                ))}
