...
function DesktopNavItem({ item }: { item: ResolvedItem }) {
  ...
  // Plain link — no children and not a dropdown group
  if (!item.children?.length && !item.isDropdown) {
    return (
...
function MobileNavItem({ item, onClose }: { item: ResolvedItem; onClose: () => void }) {
  ...
  // Dropdown group — label is a toggle button only (no navigation link)
  if (item.isDropdown) {
    return (
      <div>
        <button
          ...
          className="flex w-full items-center justify-between px-3 py-2 text-sm font-medium hover:bg-muted rounded-md"
        >
  ...
  // Regular item with children (has its own link + separate expand toggle)
  return (
    <div>
